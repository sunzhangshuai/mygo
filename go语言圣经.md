...
- **关于分号**：不需要在【语句|声明】**末尾**添加分号，除非一行上有多条语句。
...
- **风格**
  ...
  - **命名尽量短小**
    ...
    > 名字作用域大、生命周期长时，长的命名更好。

  - **包名全小写**

  - **推荐使用驼峰式**

    > 其中缩略词【ASCII|HTML】全大写。
...
- **初始化表达式**：可以是字面量或任意的表达式。
  
  - **包级别变量声明**：main函数**执行前**完成初始化。
  - **局部变量声明**：**执行到**的时候完成初始化。
...
- **单变量声明**
  ...
  3. *省略初始化表达式*：用**零值**初始化该变量。
     ...
     - 接口或引用类型「包括**slice**、**指针**、**map**、**chan**和**函数**」：nil。
...
- **指针**
  ...
  - **等值测试**：**指向同一个变量**或**全部是nil**时相等。
  - **局部变量地址安全**：可返回函数中局部变量的地址。
  ...
  - **生命周期**
    ...
    - **存储空间**：编译器自动选择在**栈**或**堆**上分配空间，并不由 `var` 或 `new` **声明变量的方式**决定。
...
- **函数可预期错误**：用最后一个返回值来传递错误信息。

  > 根据导致失败的原因来决定返回值类型。

  - *只有一个原因*：返回布尔值，通常被命名为ok。
  - *不只一个原因*：返回error类型，通常被命名为err。

- **错误处理策略**：5种方式

  1. *传播错误*。使用 `fmt.Errorf` 函数**添加上下文信息到原始错误信息**。
...
- **标准错误类型**
  ...
  - *文件结尾错误*
    ...
    > io包保证**任何由文件结束引起的读取失败**都**返回同一个错误**——io.EOF【在io包中定义】。
