...
func (e *Exercises) Task1() {
	...
	case "clock":
		...
		task1.Clock(params[1], params[2])
	case "clockWall":
		task1.ClockWall(params[1:])
	}
}
...
func (e *Exercises) Task2() {
	...
	switch opType {
	case "server":
		err = server.Run(params[1])
	case "client":
		err = client.Run(params[1])
	}
	if err != nil {
		log.Println(err)
	}
...
func (e *Exercises) Task3() {
	...
	switch opType {
	case "netcat":
		echo.Netcat(params[1])
	case "reverb":
		echo.Reverb(params[1])
	}
}
...
func (e *Exercises) Task6() {
	...
	if err != nil {
		fmt.Println("参数有误，深度需要是整数")
		return
	}

	urls := make([]Task6WorkUrl, len(params[1:]))
	// 初始化url
...
func (e *Exercises) Task7() {
	fmt.Println(mirror.Run(2, "http://gopl.io/"))
}
...
func (e *Exercises) Task12() {
	...
	case "server":
		...
		chat.StartServer(params[1])
	case "client":
		if len(params) < 3 {
			fmt.Println("参数不足")
			return
		}
		chat.StartClient(params[1], params[2])
	}
}
